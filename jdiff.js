...
var ASM_CONSTS = {
  6592: function() {FS.mkdir('/root'); FS.mount(NODEFS, { root: '/' }, '/root');},  
 6653: function($0) {let Path = require('path'); let jspath = Path.join('/root', Path.resolve(UTF8ToString($0))); let len = lengthBytesUTF8(jspath) + 1; var strOnHeap = _malloc(len); stringToUTF8(jspath, strOnHeap, len); return strOnHeap;},  
 6875: function($0) {let Path = require('path'); let jspath = Path.join('/root', Path.resolve(UTF8ToString($0))); let len = lengthBytesUTF8(jspath) + 1; var strOnHeap = _malloc(len); stringToUTF8(jspath, strOnHeap, len); return strOnHeap;},  
 7097: function($0) {let Path = require('path'); let jspath = Path.join('/root', Path.resolve(UTF8ToString($0))); let len = lengthBytesUTF8(jspath) + 1; var strOnHeap = _malloc(len); stringToUTF8(jspath, strOnHeap, len); return strOnHeap;}
};
...
/** @type {function(...*):?} */
var dynCall_iiijiijj = Module["dynCall_iiijiijj"] = createExportWrapper("dynCall_iiijiijj");
